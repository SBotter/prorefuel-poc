...
function BeforeAfterSlider() {
  ...
  // Seek a video to t and resolve when the seek is done
  ...
  useEffect(() => {
    ...
    // Wait for enough data to seek on both videos
  ...
  // Loop CLIP_START↔CLIP_END + keep lens perfectly in sync with raw (raw = master)
  useEffect(() => {
    const raw  = rawRef.current;
    const lens = lensRef.current;
    if (!raw || !lens) return;
...
export default function LensV2Page() {
  ...
  const handleVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isMP4 && !isMOV) {
      void trackError("WRONG_VIDEO_FORMAT", "Unsupported file format.", "video_upload");
...
// ── iPhone clock correction (shared with main page) ────────────────────────
